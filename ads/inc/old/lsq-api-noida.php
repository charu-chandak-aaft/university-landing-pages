...
function GetAPIData(){
	...
	$sql1 = "SELECT id, student, contact, email, course_id, qual_id, state_id, city_id, source, school_id, school_name, page_name, otp, otp_varified, remark, utm_source, utm_medium, utm_campaign, utm_content, utm_term, utm_creative, created_on, updated_on, status, gclid, utm_campaignid, utm_adgroupid, utm_creativeid, utm_matchtype, utm_device, utm_network, utm_keyword, utm_placement from data_course_lp_new where lsq=0 and created_on between '".$prevDay." 00:00:00' and '".$difference."'";
...
while($totaldatacnt>$i){
	...
	$schoolName = $totaldata[$i]['school_name'];	
	...
	$utm_campaignid = $totaldata[$i]['utm_campaignid'];
	$utm_adgroupid = $totaldata[$i]['utm_adgroupid'];
	$utm_creativeid = $totaldata[$i]['utm_creativeid'];
	$utm_matchtype = $totaldata[$i]['utm_matchtype'];
	$utm_device = $totaldata[$i]['utm_device'];
	$utm_network = $totaldata[$i]['utm_network'];
	$utm_keyword = $totaldata[$i]['utm_keyword'];
	$utm_placement = $totaldata[$i]['utm_placement'];
	...
	{"Attribute":"mx_utm_campaignid","Value":"'.$utm_campaignid.'"},
	{"Attribute":"mx_utm_adgroupid","Value":"'.$utm_adgroupid.'"},
	{"Attribute":"mx_utm_creativeid","Value":"'.$utm_creativeid.'"},
	{"Attribute":"mx_utm_matchtype","Value":"'.$utm_matchtype.'"},
	{"Attribute":"mx_utm_device","Value":"'.$utm_device.'"},
	{"Attribute":"mx_utm_network","Value":"'.$utm_network.'"},
	{"Attribute":"mx_utm_keyword","Value":"'.$utm_keyword.'"},
	{"Attribute":"mx_utm_placement","Value":"'.$utm_placement.'"},
