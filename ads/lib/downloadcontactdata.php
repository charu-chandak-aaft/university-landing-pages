...
function GetFormDataExlLp($qdata){
	...
	$sql1 = "SELECT c.id as id, c.first_name as StudentName, c.phone as PhoneNumber, c.email as EmailId, sc.name as SchoolName, c.program as CourseName, c.page as PageName, q.q_name as Qualification, s.name as StateName, c.city as CityName, c.remark as Remark, c.created_at as CreationDate, c.lsq as LsqSent
FROM contacts c 
...
function DownloadRecordsFunc($qdata){
	...
	$file = 'Export_Contact_Data.xls';
	ob_start();
	echo '<table border="1"> ';
	echo '<tr><th>SN.</th> <th>StudentName</th> <th>PhoneNumber</th> <th>EmailId</th> <th>SchoolName</th> <th>CourseName</th> <th>PageName</th> <th>Qualification</th> <th>StateName</th> <th>CityName</th> <th>Remark</th> <th>CreationDate</th> <th>LsqSent</th></tr>';
	for ($i=1; $i<=$form_data_cnt-1; $i++) {
	echo "<tr><td>".$i."</td> <td align='center'>".$form_data[$i]['StudentName']."</td> <td>".$form_data[$i]['PhoneNumber']."</td> <td>".$form_data[$i]['EmailId']."</td> <td align='center'>".$form_data[$i]['SchoolName']."</td> <td>".$form_data[$i]['CourseName']."</td> <td>".$form_data[$i]['PageName']."</td> <td align='center'>".$form_data[$i]['Qualification']."</td> <td>".$form_data[$i]['StateName']."</td> <td>".$form_data[$i]['CityName']."</td> <td>".$form_data[$i]['Remark']."</td> <td>".$form_data[$i]['CreationDate']."</td> <td>".$form_data[$i]['LsqSent']."</td></tr>";
	}
	echo '</table>';
