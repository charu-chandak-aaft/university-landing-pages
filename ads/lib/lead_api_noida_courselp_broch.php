...
function GetAPIData(){
	...
	$sql1 = "SELECT id, student, contact, email, course_id, qual_id, state_id, city_id, source, school_id, school_name, page_name, otp, otp_varified, remark, utm_source, utm_medium, utm_campaign, utm_content, utm_term, utm_creative, created_on, updated_on, status, gclid from brochure_lp where lsq=0 and created_on between '".$prevDay." 00:00:00' and '".$difference."'";
...
function GetCronUpdate($id,$val1){
	$conn = Connection(); 		
	$sql1 = "UPDATE brochure_lp SET lsq=".$val1." WHERE id=".$id;
	$result1 = $conn->query($sql1);
}
